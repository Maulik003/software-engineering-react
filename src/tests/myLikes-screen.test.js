...
test('list of liked tuit renders mocked', async () => {
    axios.get.mockImplementation(() =>
        Promise.resolve({data: {tuits: MOCKED_LIKED_TUITS}}));
    const res = await findAllTuitsLikedByUser("XYZ");
...
test('tuits lists that are rendered on my screen', async () => {
    mock.mockRestore();
    const tuits = await findAllTuitsLikedByUser("ABC")
    render(
        <HashRouter>
            <Tuits tuits={tuits}/>
        </HashRouter>);
